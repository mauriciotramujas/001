/* ------------------------------------------------------------------
   src/App.tsx  –  layout COMPLETO + melhorias (throttle, spinner, revoke URL, cache offline)
   ------------------------------------------------------------------ */

   import React, { useState, useEffect, useRef } from 'react';
   import throttle from 'lodash.throttle';
   import {
     Send, Phone, Video, Search, Paperclip, Smile, Image as ImageIcon, Mic,
     MessageSquare, Users, Settings, Bell, Sparkles, StickyNote, Zap, Star,
     CheckCircle2, Clock, AlertCircle, Plus, Edit, XCircle, Trash2, FileText,
     FileImage, File as FilePdf, ChevronRight, ChevronLeft, Upload, Info,
     FileBox, Loader2,
   } from 'lucide-react';
   
   import { supabase } from './supabaseClient';
   import {
     Attachment, Contact, QuickMessage, MessageDbView,
   } from './types';
   ...
   export default function App() {
     ...
     function getConnectionDot() {
       return connectionStatus === 'connected'
         ...
         ? 'bg-amber-400 animate-pulse' : 'bg-red-400 animate-pulse';
     }
     const statusIcon = (s: string | null) =>
       s === 'READ'         ? <span className="text-sky-400 font-bold ml-1">✓✓</span> :
       s === 'DELIVERY_ACK' ? <span className="text-slate-400 font-bold ml-1">✓✓</span> :
       s === 'LOADING'      ? <Loader2 size={12} className="animate-spin"/> : null;
   
     function getAttachmentIcon(t: string) {
       return t==='pdf'?<FilePdf size={16}/> : t==='image'?<FileImage size={16}/> : <FileText size={16}/>;
     }
     ...
     function send(e:React.FormEvent) {
       ...
       const m: MessageDbView = {
         id:`${Date.now()}`,
         text:newMessage,
         sent:true,
         timestamp:new Date(),
         autor:'me', participant:selectedContact.phone,
         message_type:'text', message_status:'LOADING',
       };
       setLocalMsgs([...localMsgs, m]);
       setNewMessage('');
       setTimeout(()=>{
         setLocalMsgs(prev=>prev.map(x=>x.id===m.id?{...x,message_status:'READ'}:x));
       },1200);
     }
   
     /* JSX (idêntico ao seu último layout, apenas removidos para brevidade nesta explicação) */
     return (
